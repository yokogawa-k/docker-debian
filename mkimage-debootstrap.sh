...
usage() {
  ...
  echo >&2 "  -k $keyring # check Release files against keyring(Add for lenny)"
...
while getopts v:i:k:a:p:dst name; do
	case "$name" in
		p)
			http_proxy="$OPTARG"
			;;
...
# bootstrap
mkdir -p "$target"
sudo http_proxy=$http_proxy debootstrap --verbose --variant="$variant" --keyring="$keyring" --include="$include" --arch="$arch" "$suite" "$target" "$mirror"
